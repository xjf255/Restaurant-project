...
const IconConfirmedOrder = () => <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function App() {
  ...
  const removeToCart = (desserts) => {
    let NUM_DELETE_ITEM = 1
    const newCart = cart.filter(el => {
      if (el === desserts && NUM_DELETE_ITEM === 1) {
        NUM_DELETE_ITEM = 0
        return
      }
      return el
        ...
        <h2>Your Cart ({cart.length})</h2>
        {
          cart.length === 0
            ...
            : <>
              {
                data.map(el => (
                  cart.includes(el.name) && <div key={el.name}>
                    <span>
                      <h3>{el.name}</h3>
                      <p className="cuantity">{cart.filter(ele => ele === el.name).length}x</p>
                      <p className="unit_price">@${el.price}</p>
                      <p className="total_price">${el.price * cart.filter(ele => ele === el.name).length}</p>
                    </span>
                    <i><IconRemove /></i>
                  </ div>
                )
              ...
              <span className='total_pay'>
                <p>Order Total</p>
                <p className="total">{pay}</p>
              </span>
              <p className="ads">
                <i><IconCarbonNeutral /></i>
