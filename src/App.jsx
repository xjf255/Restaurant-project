...
function App() {
  ...
  const removeToCart = (desserts, numItems) => {
    let NUM_DELETE_ITEM = numItems ?? -1
    const newCart = cart.filter(el => {
      if (el === desserts && NUM_DELETE_ITEM !== 0) {
        NUM_DELETE_ITEM--
        return
      }
      return el
  ...
  return (
    <>
      <Main addToCart={addToCart} removeToCart={removeToCart} cart={cart} data={data} />
      <aside>
            ...
            : <>
              {
                data.map(el => (
                  cart.includes(el.name) && <div key={el.name} className='cart--items'>
                    ...
                    <i onClick={() => removeToCart(el.name)} ><IconRemove /></i>
                  </ div>
                )
                )
              }
